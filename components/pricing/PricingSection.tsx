...
const FEATURES = [
  ...
  { label: "Custom build request",  free: false, pro: true,        lifetime: true },
  { label: "Early access to tools", free: false, pro: false,       lifetime: true },
...
export function PricingSection() {
  ...
  async function handleSelectPro() {
    ...
    const priceId = billing === "monthly" ? PLANS.MONTHLY.stripeId : PLANS.YEARLY.stripeId
    setLoadingPlan("pro")
    try {
  ...
  async function handleSelectLifetime() {
    ...
    try {
      const res = await fetch("/api/stripe/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ priceId: PLANS.LIFETIME.stripeId, mode: "payment" }),
      })
      const data = await res.json()
      if (data.url) window.location.href = data.url
    } catch (err) {
    ...
    <div>
      ...
      <div className="flex items-center justify-center gap-4 mb-14">
        ...
        <button
          ...
          style={{ color: billing === "yearly" ? "var(--text-primary)" : "var(--text-muted)" }}
        >
          Yearly
          <span
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border-default)" }}>
                <th className="text-left p-5 text-sm font-semibold text-[var(--text-muted)]">Feature</th>
                <th className="p-5 text-center text-sm font-bold text-[var(--text-primary)]">Free</th>
                <th
                  className="p-5 text-center text-sm font-bold"
                  style={{ background: "rgba(0,102,255,0.06)", color: "var(--accent-blue)" }}
                >
                  Pro
                </th>
                <th className="p-5 text-center text-sm font-bold text-[var(--text-primary)]">Lifetime</th>
              </tr>
            </thead>
            <tbody>
              {FEATURES.map((feature, i) => (
                ...
                >
                  <td className="p-5 text-sm text-[var(--text-secondary)]">{feature.label}</td>
                  <td className="p-5 text-center"><FeatureCell value={feature.free} /></td>
                  <td className="p-5 text-center" style={{ background: "rgba(0,102,255,0.04)" }}>
                    <FeatureCell value={feature.pro} />
                  </td>
                  <td className="p-5 text-center"><FeatureCell value={feature.lifetime} /></td>
                </tr>
              ))}
            </tbody>
          </table>
